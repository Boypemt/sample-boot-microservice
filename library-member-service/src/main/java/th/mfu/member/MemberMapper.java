...
/**
 * The Assembler for Member. Same idea, same annotations, as the 3-tier sample -
 * you write the interface, MapStruct writes the class at compile time.
 * <p>
 * After a build, go and read what it produced:
 *
 * <pre>
 *   library-member-service/target/generated-sources/annotations/
 *       th/mfu/member/MemberMapperImpl.java
 * </pre>
 *
 ...
 * relationship to flatten here, because Member has none: the only thing it could
...
public interface MemberMapper {
    ...
    /**
     * Two sources, one DTO. id, name, email and age come from the member; the
     * port is handed in, because it is not a property of the member at all - it
    ...
    MemberDTO toDto(Member member, int port);
}
